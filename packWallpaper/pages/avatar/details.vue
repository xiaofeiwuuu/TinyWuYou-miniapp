<template>
	<page-layout customClass="navbar__content">
		...
		<view class="avatar-box">
			<view class="avatar-box__now">
				<view class="avatar-box__now--image">
			...
			<view class="fu-m-t-30">
				<jc-section title="推荐头像" :showRight="false" />
...
<script setup>
	...
	// data数据
	const { $parseURL, $openPage } = getCurrentInstance().appContext.config.globalProperties;
	let data = ref({
		image: '',
		title: '',
		id: null
	...
	onLoad(async (options) => {
		...
		if (imageId) {
			...
			console.error('[AvatarDetails] 缺少图片ID参数');
		}
	});
	...
	const loadImageDetail = async (imageId) => {
		try {
			console.log('[AvatarDetails] 加载图片详情:', imageId);
			...
			if (res.code === 0) {
				...
				console.log('[AvatarDetails] 图片详情加载成功');

				// 检查收藏状态
				await loadCollectionStatus(imageId);
			} else {
				console.error('[AvatarDetails] 加载失败:', res.message);
			}
		} catch (error) {
			console.error('[AvatarDetails] 加载异常:', error);
		}
	};
	...
	const loadCollectionStatus = async (imageId) => {
		try {
			...
			if (res.code === 0) {
				...
				console.log('[AvatarDetails] 收藏状态:', isCollected.value);
			}
		} catch (error) {
			console.error('[AvatarDetails] 加载收藏状态失败:', error);
	...
	const loadRecommendList = async () => {
		try {
			console.log('[AvatarDetails] 加载推荐头像列表');

			const res = await getRecommendImages('avatar', 9);
			...
			if (res.code === 0) {
				...
				console.log('[AvatarDetails] 推荐头像列表加载成功:', list.value.length);
			}
		} catch (error) {
			console.error('[AvatarDetails] 加载推荐头像列表异常:', error);
		}
	};
	...
	const handleImageClick = (item) => {
		...
		if (item.id === data.value.id) {
			console.log('[AvatarDetails] 点击的是当前图片,不跳转');
			return;
		}
	...
	onShareAppMessage(() => {
		return {
			title: data.value.title || '精美头像分享',
			path: `/packWallpaper/pages/avatar/details?query=${encodeURIComponent(JSON.stringify({ imageId: data.value.id }))}`,
