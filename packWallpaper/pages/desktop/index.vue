<template>
	<page-layout>
		...
		<view class="fu-m-x-30 fu-m-t-20" style="color: #FFFFFF;">
			<jc-grid :list="list" :column="2" multiple="0.7" @click="handleImageClick" />
...
<script setup>
	...
	onLoad(async (options) => {
		...
		console.log('[DesktopIndex] 页面参数:', { title: title.value, type: imageType.value });
	...
	// tabs切换
	const handleTabs = (e) => {
		console.log('[DesktopIndex] 切换分类:', e);
	...
	const initList = async () => {
		if (!currentCategoryId.value) {
			console.log('[DesktopIndex] 分类ID为空，跳过加载');
			return;
		}
		...
		try {
			console.log('[DesktopIndex] 加载图片列表:', {
			...
			if (res.code === 0) {
				...
				console.log('[DesktopIndex] 加载成功，当前列表长度:', list.value.length);
			} else {
				console.error('[DesktopIndex] 加载失败:', res.message);
				queryParams.value.loadingType = 3; // 加载失败
			}
		} catch (error) {
			console.error('[DesktopIndex] 加载异常:', error);
			queryParams.value.loadingType = 3; // 加载失败
		}
	};
	...
	const handleImageClick = (item) => {
		console.log('[DesktopIndex] 点击图片:', item);
		if (item && item.id) {
			$openPage({
				name: 'desktopDetails',
			...
			console.error('[DesktopIndex] 图片数据缺少ID:', item);
		}
	};
</script>
