...
<script setup>
	...
	let title = ref('精美贴纸');
	let imageType = ref('sticker'); // 当前图片类型
	...
	onLoad(async (options) => {
		console.log('[StickerIndex] 页面参数:', { title: title.value, type: imageType.value });
	...
	// tabs切换
	const handleTabs = (e) => {
		console.log('[StickerIndex] 切换分类:', e);
	...
	const initList = async () => {
		if (!currentCategoryId.value) {
			console.log('[StickerIndex] 分类ID为空，跳过加载');
			return;
		}
		...
		try {
			console.log('[StickerIndex] 加载图片列表:', {
			...
			if (res.code === 0) {
				...
				console.log('[StickerIndex] 加载成功，当前列表长度:', list.value.length);
			} else {
				console.error('[StickerIndex] 加载失败:', res.message);
				queryParams.value.loadingType = 3; // 加载失败
			}
		} catch (error) {
			console.error('[StickerIndex] 加载异常:', error);
			queryParams.value.loadingType = 3; // 加载失败
		}
	};
	...
	const handleImageClick = (item) => {
		console.log('[StickerIndex] 点击图片:', item);
		if (item && item.id) {
			$openPage({
				name: 'stickerDetails',
			...
			console.error('[StickerIndex] 图片数据缺少ID:', item);
		}
	};
</script>
