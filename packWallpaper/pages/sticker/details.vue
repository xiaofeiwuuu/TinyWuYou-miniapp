	...
	<page-layout customClass="navbar__content">
		...
		<view class="sticker-box">
			...
			<view v-if="list.length > 0" class="fu-m-t-30">
				<jc-section title="推荐贴纸" :showRight="false" />
...
<script setup>
	...
	onLoad(async (options) => {
		...
		if (imageId) {
			...
			console.error('[StickerDetails] 缺少图片ID参数');
		}
	});
	...
	const loadImageDetail = async (imageId) => {
		try {
			console.log('[StickerDetails] 加载图片详情:', imageId);
			...
			if (res.code === 0) {
				...
				console.log('[StickerDetails] 图片详情加载成功');

				// 检查收藏状态
				await loadCollectionStatus(imageId);
			} else {
				console.error('[StickerDetails] 加载失败:', res.message);
			}
		} catch (error) {
			console.error('[StickerDetails] 加载异常:', error);
		}
	};
	...
	const loadCollectionStatus = async (imageId) => {
		try {
			...
			if (res.code === 0) {
				...
				console.log('[StickerDetails] 收藏状态:', isCollected.value);
			}
		} catch (error) {
			console.error('[StickerDetails] 加载收藏状态失败:', error);
	...
	const loadRecommendList = async () => {
		try {
			console.log('[StickerDetails] 加载推荐贴纸列表');

			const res = await getRecommendImages('sticker', 9);
			...
			if (res.code === 0) {
				...
				console.log('[StickerDetails] 推荐贴纸列表加载成功:', list.value.length);
			}
		} catch (error) {
			console.error('[StickerDetails] 加载推荐贴纸列表异常:', error);
		}
	};
	...
	const handleImageClick = (item) => {
		...
		if (item.id === data.value.id) {
			console.log('[StickerDetails] 点击的是当前图片,不跳转');
			return;
		}
	...
	onShareAppMessage(() => {
		return {
			title: data.value.title || '精美贴纸分享',
			path: `/packWallpaper/pages/sticker/details?query=${encodeURIComponent(JSON.stringify({ imageId: data.value.id }))}`,
